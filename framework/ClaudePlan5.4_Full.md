...
> **v5.4 增强**：`manage_service.action` 新增 `reload`（非关键服务 WARN-LOW）和 `daemon-reload`（WARN-LOW）。
...
> **v5.4 增强**：`manage_firewall.action` 新增 `reload`，覆盖防火墙配置刷新场景。
...
| 29 | `manage_sysctl` | 内核参数 | `action(list|get|set|apply-file)`, `key`, `value`, `persist` | SAFE(list/get) / WARN-HIGH |
| 32 | `manage_archive` | 归档压缩 | `action(list|create|extract)`, `archive_path`, `target_path`, `format(auto|tar|tar.gz|zip)`, `strip_components` | SAFE(list) / WARN-LOW(create) / WARN-HIGH(extract) / BLOCK |
...
| ID | 触发条件 | 适用工具 |
|---|---|---|
| WL006 | `read_file(path)` 匹配 `/etc/*`（非 BLOCK 路径） | `read_file` |
| WL007 | `write_file(mode=create_only)` 且目标路径不在 PERSISTENCE_ENTRY_PATHS | `write_file` |
| WL008 | `copy_move_path(action=copy)` | `copy_move_path` |
...
steps:
  - id: 步骤ID
    type: tool_call | confirm | display | input
    depends_on: [依赖步骤ID]
...
steps:
  ...
  - {id: s9, type: tool_call, tool: check_endpoint, args: "{{verify_endpoint}}", depends_on: [s8], condition: "{{verify_endpoint is not none}}", on_fail: rollback}

rollback:
  - {id: r1, type: tool_call, tool: backup_path, args: {action: restore, backup_id: "{{s5.result.backup_id}}", path: "{{file_path}}"}}
...
parameters:
  ...
  - {name: host_port, type: text, required: true}
  - {name: container_port, type: text, required: true}
steps:
  ...
  - {id: s3, type: tool_call, tool: manage_container,
     ...
     depends_on: [s2]}
  - {id: s4, type: tool_call, tool: check_endpoint, args: {kind: tcp, host: localhost, port: "{{host_port}}"}, depends_on: [s3]}
...
parameters:
  ...
  - {name: endpoint_port, type: text, required: true}
...
P0（最优先，核心通路与基础工具）
  ...
  RiskClassifier（B001-B027 / WH001-WH021 / WL001-WL015）
  ...
  核心工具实现（P0 先行）：
    ...
    SENSITIVE_CREDENTIAL_PATHS / PERSISTENCE_ENTRY_PATHS / CRITICAL_EDIT_PATHS 集合注册
...
P1（高频运维增强）
  ...
  CommandSafetyChecker（CS001-CS009 + 委托 CS010-CS014）
