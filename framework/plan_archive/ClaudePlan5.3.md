# SysDialogue — v5.3 扩展设计文档

> 基于 v4.1，新增：扩充工具面（12 → 21，新增 9 个）+ 动态工具创建机制  
> 本文档只描述**相对 v4.1 的增量变更**，原有设计不变  
> v5.3 相对 v5.2 的修订（按优先级）：  
> P1-1 manage_firewall schema 条件约束、P1-2 远程锁门规则 ID 统一（废弃 CS010-CS014）、  
> P2-1 RiskClassifier 去副作用、P2-2 防火墙常见语义映射补全
...
不同后端语法差异大，误判/漏判概率高。v5.2 改为结构化参数，v5.3 进一步补全 schema 条件约束：
...
```json
{
  ...
  "input_schema": {
    ...
    "allOf": [
      {
        "if":   {"properties": {"action": {"enum": ["flush","list"]}}, "required": ["action"]},
...
**条件约束说明（`allOf + if/then + not + required` 组合）：**

| action | required | 禁止出现 | target 内部约束 |
|---|---|---|---|
| `flush` / `list` | — | `target`、`policy` | — |
| `set-default` | `policy` | `target` | — |
| `allow` / `deny` / `delete` | `target` | `policy` | `port` 或 `service` 至少一个 |
...
新模块 `security/remote_lockout.py`，是所有远程锁门判定的**唯一实现来源**。  
两条执行路径（静态工具 / 动态工具）均调用此模块，消除双轨逻辑漂移。

**规则 ID 命名空间：** 所有返回值统一使用 B 系规则号（B010/B015-B017）。  
动态工具通路经 `assess_cmd()` 触发时返回的仍是 B 系 ID，会直接出现在  
`SafetyCheckResult.flags` 中，这是预期行为。CS010-CS014 编号不再使用。

```python
@dataclass
class LockoutRisk:
    level: str              # "BLOCK" | "WARN-HIGH" | "SAFE"
    rule_id: str            # "B010" / "B015" / "B016" / "B017"（统一 B 系）
...
def assess_cmd(cmd: list[str], env_profile: EnvProfile) -> LockoutRisk:
    """供 CommandSafetyChecker 调用，命令为原始 argv。
    优先经 StaticRuleMapper 映射后复用 assess_tool；映射失败时兜底正则。
    返回值 rule_id 统一为 B 系（B010/B015-B017），不使用 CS 编号。
    """
...
def _check_raw_lockout_patterns(cmd: list[str], env_profile: EnvProfile) -> LockoutRisk:
    """映射失败时的兜底：只匹配最明确的远程锁门命令形态，rule_id 仍用 B 系"""
    if not env_profile.remote_mode:
        return LockoutRisk("SAFE", "", "", False)
    flat = " ".join(cmd)
    # 等价 B015：清空规则
    ...
    if re.search(rf'--dport\s+{port_str}\b.*\b(DROP|REJECT)\b', flat) or \
       re.search(rf'\bufw\b\s+deny\s+(ssh|{port_str})\b', flat):
        return LockoutRisk("BLOCK", "B017", f"封锁 SSH 端口 {port_str}（远程模式）", True)
    return LockoutRisk("SAFE", "", "", False)
```
...
- `CommandSafetyChecker.check_command()` 调用 `RemoteLockoutChecker.assess_cmd()`，  
  结果合并进 `SafetyCheckResult`；flags 中出现 B 系 ID 是预期行为。
- 两条路径不再各自维护匹配逻辑，B010/B015-B017 是唯一规则 ID 命名空间。

### 3.5 CommandSafetyChecker

模块 `security/command_safety.py`，远程锁门检测委托 RemoteLockoutChecker：
...
def check_command(cmd: list[str], env_profile: EnvProfile) -> SafetyCheckResult:
    ...
    # 远程锁门：委托 RemoteLockoutChecker，返回 B010/B015-B017 ID
    lockout = RemoteLockoutChecker.assess_cmd(cmd, env_profile)
    if lockout.level != "SAFE":
        flags.append(lockout.rule_id)   # B010 / B015 / B016 / B017
...
| — | 远程锁门：委托 `RemoteLockoutChecker.assess_cmd()`，命中时 flags 中出现 B010/B015-B017 | BLOCK |

CS010-CS014 编号不再使用；命令层远程锁门检测的规则 ID 与静态工具路径统一为 B 系。
...
**StaticRuleMapper — 有限白名单，防火墙条目产出结构化 target（v5.3 补全常见变体）：**
...
WHITELIST_MAPPINGS = [
    ...
    (["yum",    "remove",  PKG],    → manage_package(name=PKG, action="remove",  manager="yum")),
    (["dnf",    "remove",  PKG],    → manage_package(name=PKG, action="remove",  manager="dnf")),

    # ── ufw ────────────────────────────────────────────────────────────
    # ufw allow/deny/reject <port|service>  或  <port>/<proto>
    (["ufw", "allow",  T],          → manage_firewall(backend="ufw", action="allow",  target=_ufw_t(T))),
    (["ufw", "deny",   T],          → manage_firewall(backend="ufw", action="deny",   target=_ufw_t(T))),
    (["ufw", "reject", T],          → manage_firewall(backend="ufw", action="deny",   target=_ufw_t(T))),
    # ufw delete allow/deny <port|service>
    (["ufw", "delete", "allow", T], → manage_firewall(backend="ufw", action="delete", target=_ufw_t(T))),
    (["ufw", "delete", "deny",  T], → manage_firewall(backend="ufw", action="delete", target=_ufw_t(T))),
    # ufw allow/deny from <src> to any port <port>
    (["ufw", ACT, "from", SRC, "to", "any", "port", PORT],
                                    → manage_firewall(backend="ufw", action=_ufw_act(ACT),
                                        target={source_ip:SRC, port:int(PORT)})),
    # ufw default deny/allow/reject
    (["ufw", "default", POLICY],    → manage_firewall(backend="ufw", action="set-default",
                                        policy=_map_policy(POLICY))),

    # ── iptables ───────────────────────────────────────────────────────
    # iptables -A/-I <chain> -p <proto> --dport <port> -j <TARGET>
    (["iptables", ADD, CHAIN, "-p", PROTO, "--dport", PORT, "-j", TGT],
                                    → manage_firewall(backend="iptables",
                                        action=_ipt_action(TGT),
                                        target={port:int(PORT), protocol:PROTO},
                                        direction=_ipt_dir(CHAIN))),
    # iptables -D <chain> -p <proto> --dport <port> -j <TARGET>
    (["iptables", "-D", CHAIN, "-p", PROTO, "--dport", PORT, "-j", TGT],
                                    → manage_firewall(backend="iptables", action="delete",
                                        target={port:int(PORT), protocol:PROTO})),
    # iptables -A/-I <chain> -s <src> -j <TARGET>
    (["iptables", ADD, CHAIN, "-s", SRC, "-j", TGT],
                                    → manage_firewall(backend="iptables",
                                        action=_ipt_action(TGT),
                                        target={source_ip:SRC},
                                        direction=_ipt_dir(CHAIN))),
    # iptables -P <chain> <POLICY>  （修改链默认策略）
    (["iptables", "-P", CHAIN, POL],
                                    → manage_firewall(backend="iptables", action="set-default",
                                        policy=_ipt_policy(POL),
                                        direction=_ipt_dir(CHAIN))),

    # ── firewall-cmd ───────────────────────────────────────────────────
    # --add-port=22/tcp  /  --remove-port=22/tcp
    (["firewall-cmd", "--add-port=*"],
                                    → manage_firewall(backend="firewalld", action="allow",
                                        target=_fcmd_port(a[1]))),
    (["firewall-cmd", "--remove-port=*"],
                                    → manage_firewall(backend="firewalld", action="delete",
                                        target=_fcmd_port(a[1]))),
    # --add-service=ssh  /  --remove-service=ssh
    (["firewall-cmd", "--add-service=*"],
                                    → manage_firewall(backend="firewalld", action="allow",
                                        target={service:_fcmd_svc(a[1])})),
    (["firewall-cmd", "--remove-service=*"],
                                    → manage_firewall(backend="firewalld", action="delete",
                                        target={service:_fcmd_svc(a[1])})),
    # --set-default-policy=DROP/ACCEPT
    (["firewall-cmd", "--set-default-policy=*"],
                                    → manage_firewall(backend="firewalld", action="set-default",
                                        policy=_fcmd_policy(a[1]))),
]

# 解析辅助函数（有限覆盖，不做通用命令解析）：
# _ufw_t("22/tcp") → {port:22, protocol:"tcp"}
# _ufw_t("ssh")    → {service:"ssh"}
# _ufw_t("22")     → {port:22}
# _ufw_act("allow"|"deny"|"reject") → "allow"|"deny"
# _map_policy("deny"|"reject") → "drop"|"reject"；"allow" → "accept"
# _ipt_action("DROP"|"REJECT") → "deny"；"ACCEPT" → "allow"
# _ipt_dir("INPUT") → "in"；"OUTPUT" → "out"；"FORWARD" → "forward"
# _ipt_policy("DROP") → "drop"；"ACCEPT" → "accept"
# _fcmd_port("--add-port=22/tcp") → {port:22, protocol:"tcp"}
# ADD = "-A" | "-I"（插入）；未在此表中的命令：semantic_unmapped=True
```
...
不修改核心逻辑。`RiskClassifier` 保持纯判定器，无副作用：

```python
# RiskClassifier.classify() 返回值扩展 lockout_warning 字段
@dataclass
class RiskResult:
    level: str
    rule_id: str
    reason: str
    lockout_warning: bool = False  # 调用方负责追加 decision_trace，classifier 不写
...
if lockout.level == "BLOCK":
    return RiskResult("BLOCK", lockout.rule_id, lockout.explanation, True)
# 非 BLOCK 时把 self_lockout_warning 透传给调用方
risk_result = _apply_existing_rules(tool_name, args)
risk_result.lockout_warning = lockout.self_lockout_warning
return risk_result
```

```python
# 流程层（ClaudeClient / ToolExecutionHandler）负责追加 decision_trace
rc = RiskClassifier.classify(tool_name, args, env_profile)
if rc.lockout_warning:
    decision_trace.append(f"lockout_warning({rc.rule_id or 'remote_firewall'})")
```
...
| 远程锁门逻辑双轨漂移 | 静态/动态两条路各自维护匹配逻辑 | 统一委托 RemoteLockoutChecker；CS010-CS014 编号废弃，flags 中统一出现 B 系 ID |
| RiskClassifier 写入 decision_trace 副作用 | 判定器混入审计逻辑 | RiskClassifier 只返回 RiskResult（含 lockout_warning 字段）；流程层负责追加 decision_trace |
| manage_firewall schema 歧义参数组合 | 只 required action，其余无约束 | allOf+if/then+not+required 条件约束：flush/list 禁 target/policy；set-default 必须 policy；allow/deny/delete 必须 target |
| manage_firewall 结构化参数后端翻译错误 | firewall.py 实现 | 单元测试覆盖 ufw/firewalld/iptables 各后端的参数→命令翻译，与 RemoteLockoutChecker 测试独立 |
| StaticRuleMapper 防火墙映射覆盖不足 | ufw/iptables 真实变体多 | v5.3 补全 ufw delete、from src、iptables -I/-D/-P、firewall-cmd add/remove-port/service；未覆盖变体仍走 UNKNOWN confirm + 兜底正则 |
...
v5.3 在 v5.2 基础上再收紧四个缺口：

- **manage_firewall schema 条件约束**（P1-1）：`allOf+if/then+not+required` 组合，  
  按 action 类型精确声明 required/禁止字段，schema 层拦截歧义参数组合，不推迟到实现层
- **远程锁门规则 ID 统一**（P1-2）：废弃 CS010-CS014 编号，`assess_cmd()` 统一返回  
  B010/B015-B017，两条执行路径的 flags 和 AuditLog 使用同一命名空间
- **RiskClassifier 去副作用**（P2-1）：`RiskClassifier` 保持纯判定器，`lockout_warning`  
  通过 `RiskResult` 字段透传给流程层，`decision_trace.append()` 由调用方执行
- **防火墙映射补全**（P2-2）：StaticRuleMapper 新增 ufw delete/from-src、iptables -I/-D/-P、  
  firewall-cmd add/remove-port/service 等常见变体，减少 `semantic_unmapped=true` 的频率
