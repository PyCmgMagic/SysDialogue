...
├── __init__.py
├── agent/                       ✅ 完成
│   ├── __init__.py              # 导出 AgentController / ClaudeClient / build_system_prompt
│   ├── controller.py            # AgentController + ClaudeClient（agentic loop + 安全门 + 元工具路由）
...
# 覆盖模型
python -m sysdialogue.app.cli --model claude-opus-4-7
```
...
AI 层（ClaudeClient / Anthropic SDK）
  ↓ tool_use 块
...
**闭环**：自然语言 → 意图解析（由 Claude）→ 环境感知（EnvProfile）→ 风险判定 → 计划生成/工作流匹配 → 预览/确认 → 执行 → 验证 → 回滚或完成 → 自然语言反馈 → 审计记录。
...
c27741e feat(agent): SystemPromptBuilder + ClaudeClient + AgentController
...
1. **真机 Linux 端到端验证**：
   - Ubuntu 22.04 本地 + 真 ANTHROPIC_API_KEY
