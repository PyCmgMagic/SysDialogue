...
交互式入口需要模型接口配置。提交材料不包含真实密钥，现场通过环境变量或 `.env` 注入。
...
启动 TUI（须在linux服务器上使用）：

```powershell
python -m sysdialogue.app.cli
```

远程 SSH 目标机（可在windows上启用，远程连接linux服务器）：
...
7. 若有远程 Linux 靶机，使用 `--remote` 展示远程执行与 SSH 防锁门。
8. 若需要展示复杂动态命令，显式使用 `--break-glass` 启动。
...
```text
submission_materials/evidence/verification_log_2026-04-25.md
```
